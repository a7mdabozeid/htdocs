...
AppBar customAppBar ({required title,Widget? leading,List<Widget>? actions,int elevation = 3,Widget? bottomChild,Color? backgroundColor,bottomChildHeight,leadingWidth,toolbarHeight}){
  return AppBar(
    backgroundColor: backgroundColor??AppUI.whiteColor,
    elevation: double.parse(elevation.toString()),
    toolbarHeight: toolbarHeight,
    title: title is Widget? title : CustomText(text: title, fontSize: 18.0,color: AppUI.blackColor,fontWeight: FontWeight.w500,),
    centerTitle: true,
    leading: leading,
...
class CustomAppBar extends StatelessWidget {
  ...
  const CustomAppBar({Key? key,required this.title,this.leading,this.onBack}) : super(key: key);
          ...
          child: Row(
            ...
            children: [
              InkWell(
                  ...
                  child: Icon(Icons.arrow_back_ios,color: AppUI.blackColor,size: 19,)),
...
class LoadingWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Lottie.asset('assets/lottie/loading.json', height: 90,width: 90, fit: BoxFit.fill),
    );
  }
...
class ProductCard extends StatelessWidget {
  ...
  const ProductCard({Key? key,this.type = "grid",this.buttonText, this.product,this.onFav,this.onDelete, this.from,this.addToCart,this.onTap}) : super(key: key);
          ...
          Expanded(
            ...
            child: Stack(
              ...
              children: [
                ClipRRect(
                  borderRadius: BorderRadius.circular(10),
                  child: CachedNetworkImage(imageUrl: product!.images!=null?product!.images!.isEmpty?"":product!.images![0].src!:"",fit: BoxFit.fill,height: 270,width: double.infinity,placeholder: (context, url) => Stack(
                    children: [
                      Image.asset("${AppUI.imgPath}product_background.png",height: 270,width: double.infinity,fit: BoxFit.fill,),
                          ...
                          InkWell(
                            onTap: onFav,
                              child: Padding(
                                ...
                                child: Icon(product!.fav!?Icons.favorite:Icons.favorite_border,color: AppUI.errorColor,size: 20,),
                              ),)
                        ],
                      )
                    ],
                ...
                children: [
                  Column(
                    ...
                    children: [
                      ...
                      // const SizedBox(height: 5,),
                      CustomText(text: "${product!.salePrice==""?product!.price:product.salePrice} SAR",color: product!.salePrice==""?AppUI.mainColor:AppUI.orangeColor,fontWeight: FontWeight.w600,),
                      if(product!.salePrice!="")
                      CustomText(text: "${product!.regularPrice==""?product!.price:product!.regularPrice} SAR",color: AppUI.iconColor,textDecoration: TextDecoration.lineThrough,fontSize: 12,),
                      CustomText(text: product!.name!.length>20?product!.name!.substring(0,20):product!.name,color: AppUI.blackColor,fontSize: 12,),
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Expanded(
                flex: 2,
                child: CachedNetworkImage(imageUrl: product!.image!= null ?product!.image.src!:"",placeholder: (context, url) => Stack(
                  children: [
              ...
              Expanded(
                flex: 2,
                child: CachedNetworkImage(imageUrl: product.image!=null?product.image!['src']!:product.images!=null && product.images!.isNotEmpty?product.images![0].src!:"",height: 130,placeholder: (context, url) => Stack(
                  children: [
                        ...
                        children: [
                          SizedBox(
                            width: AppUtil.responsiveWidth(context)*0.6,
                              child: CustomText(text: type.split(" ")[0] == "order"?product!.name:product!.name,color: AppUI.blueColor,)
                          ),
                          const SizedBox(height: 5,),
                          CustomText(text: type.split(" ")[0] == "order"?'':product!.description!.length<3?product!.description:"${product!.description!.substring(3,product!.description!.length>29?24:product!.description!.length-5)}...",color: AppUI.blackColor,),
                          const SizedBox(height: 5,),
                          if(type.split(" ")[0] == "order")
                          CustomText(text: "${product!.price.toString().length>6?product!.price.toString().substring(0,6):product!.price.toString()} SAR",color: AppUI.mainColor,fontWeight: FontWeight.w600,)
                          else
                          CustomText(text: "${product!.salePrice==""? product!.price:product!.salePrice} SAR",color:  product!.salePrice==""?AppUI.mainColor:AppUI.orangeColor,fontWeight: FontWeight.w600,),
                          if(type.split(" ")[0] != "order")
                            if(product!.salePrice!="")
                          CustomText(text: "${product!.price} SAR",color: AppUI.iconColor,textDecoration: TextDecoration.lineThrough,fontSize: 12,),
                          const SizedBox(height: 5,),
                          if(type.split(" ")[0] != "order")
                            if(product.attributes.isNotEmpty && product.attributes[0].option!=null)
                          Row(
                            children: [
                              CustomText(text: product.attributes[0].name,color: AppUI.iconColor,),
                              const SizedBox(width: 10,),
                              CustomText(text: product.attributes[0].option,color: AppUI.blackColor,),
                            ],
                          ),
                          // SizedBox(
                          //   height: 25,
            ...
            child: Row(
              children: [
                InkWell(
                  ...
                  child: Icon(Icons.share_outlined,color: AppUI.blackColor,size: 19,),
                ),
                Container(
                  margin: const EdgeInsets.symmetric(horizontal: 10),
                  height: 20,width: 1,color: AppUI.greyColor,
                ),
                if(type.split(" ")[0] != "order" || type.split(" ")[0] == "order" && type.split(" ")[1] == "current")
                  InkWell(
                      onTap: onDelete,
                      child: Image.asset("${AppUI.imgPath}trash_red.png",height: 19,)),
                const Spacer(),
                InkWell(
                    onTap: addToCart,
                    child: Row(
                      children: [
                        Image.asset("${AppUI.imgPath}bag.png",color: AppUI.mainColor,height: 19,),
                        const SizedBox(width: 10,),
