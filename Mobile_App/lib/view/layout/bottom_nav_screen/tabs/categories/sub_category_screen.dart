        ...
        child: Column(
          children: [
            ...
            Padding(
              ...
              child: Row(
                children: [
                  CustomText(text: "shopByProducts".tr(),fontWeight: FontWeight.w500,),
                ],
              ),
            ),
            Container(
              ...
              child: BlocBuilder<CategoriesCubit,CategoriesState>(
                ...
                builder: (context, states) {
                  ...
                  return Column(
                    children: List.generate(cubit.subSubCategoriesModel.length, (index) {
                      return Padding(
                        padding: const EdgeInsets.all(8.0),
                        child: ListTile(
                          onTap: (){
                            AppUtil.mainNavigator(context, ProductsScreen(catId: cubit.subSubCategoriesModel[index].id!, catName: cubit.subSubCategoriesModel[index].name!,));
                          },
                          leading: CircleAvatar(
                            backgroundColor: AppUI.backgroundColor,
                            child: ClipRRect(
                              borderRadius: BorderRadius.circular(50),
                              child: CachedNetworkImage(imageUrl: cubit.subSubCategoriesModel[index].image==null?"":cubit.subSubCategoriesModel[index].image!.src!,placeholder: (context, url) => Image.asset("${AppUI.imgPath}men.png",),
                                errorWidget: (context, url, error) => Image.asset("${AppUI.imgPath}men.png",),),
                            ),
                          ),
                          title: CustomText(text: cubit.subSubCategoriesModel[index].name),
                        ),
                      );
                    }),
                  );
