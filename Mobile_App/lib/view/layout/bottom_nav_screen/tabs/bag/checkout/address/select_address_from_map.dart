...
class _SelectAddressFromMapState extends State<SelectAddressFromMap> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: widget.lat==null?null:customAppBar(title: "location".tr()),
        body: _currentLocation!=null?Stack(
          children: [
            GoogleMap(
              onMapCreated: (map) async {
                ...
                googleMapController!.animateCamera(
                    CameraUpdate.newCameraPosition(
                        CameraPosition(target: LatLng(_currentLocation!.latitude, _currentLocation!.longitude), zoom: 1)
                    )
                );
              ...
              initialCameraPosition: CameraPosition(target: LatLng(_currentLocation!.latitude,_currentLocation!.longitude,),zoom: 1),
              // myLocationButtonEnabled: true,
              myLocationEnabled: true,
              myLocationButtonEnabled: false,
              markers: widget.lng!=null?{Marker(markerId: MarkerId(widget.lat.toString()),position: LatLng(widget.lat!, widget.lng!))}: {},
            ),
              Align(
                          ...
                          child: InkWell(
                            onTap: (){
                              print('vjkndfjkv');
                                  googleMapController!.animateCamera(
                                  CameraUpdate.newCameraPosition(
                                      CameraPosition(target: LatLng(posision!.latitude, posision!.longitude), zoom: 8)
                                  )
                              );
                            },
                            child: CircleAvatar(
                              ...
                              child: SvgPicture.asset("${AppUI.iconPath}location.svg",color: AppUI.orangeColor,height: 30,width: 30,),
                            ),
                          ),
                          // child: CustomCard(
                          //   child: Row(
            ...
            if(widget.lat == null)
              SizedBox.expand(
                child: Center(
                    child: Icon(Icons.add_location,color: AppUI.orangeColor,size: 50,)
                ),
              ),
            if(widget.lat == null)
              Positioned(
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      CustomButton(text: "select".tr(), onPressed: () async {
                        address = await AppUtil.getAddress(LatLng(_currentLocation!.latitude, _currentLocation!.longitude));
                        if(address[2].toString().toLowerCase()!="sa"){
                          AppUtil.errorToast(context, "youMustBeInSaudiArabia".tr());
                          return;
                        }
                        if(!mounted)return;
