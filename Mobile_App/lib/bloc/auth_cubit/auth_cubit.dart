...
class AuthCubit extends Cubit<AuthState> {
  ...
  register(context) async {
    ...
    try{
      ...
      if(response['id'] == null){
        ...
        await ProfileCubit.get(context).updatePhone(email: registerEmail.text,phone: registerPhone.text);
        ...
        Map<String,dynamic> sendCodeResponse = await sendCode(registerEmail.text,context);
        if(sendCodeResponse['data']['status'] == 200){
          reset();
          AppUtil.successToast(context, "codeSentSuccessfully".tr());
          AppUtil.mainNavigator(context, ForgotPass2(email: registerEmail.text));
        }else{
  ...
  Future<Map<String,dynamic>> sendCode(email,context) async {
    ...
    try{
      ...
      if(response['data']['status']==200){
        AppUtil.mainNavigator(context, ForgotPass2(email: forgotPassEmail.text,type: "forgot",));
        AppUtil.successToast(context, "codeSentSuccessfully".tr());
      }else{
        AppUtil.errorToast(context, "someThingWentWrong".tr());
      }
  ...
  Future<Map<String,dynamic>> verifyCode(context,email, {type}) async {
    emit(ResetPassLoadingState());
    try{
      Map<String,dynamic> response = await AuthRepositories.validateCode(email,verifyController1.text);
      if(response['data']['status'] == 200){
  ...
  Future<Map<String,dynamic>> resetPassword(context) async {
    emit(ResetPassLoadingState());
    try{
      Map<String,dynamic> response = await AuthRepositories.resetPassword(forgotPassEmail.text,resetPass.text,verifyController1.text);
      if(response['data']['status'] == 200){
        reset();
        await AppUtil.successToast(context, "passwordChangedSuccessfully".tr());
        AppUtil.removeUntilNavigator(context, const AuthScreen(initialIndex: 1));
        login(context);
      }else{
        AppUtil.errorToast(context, "someThingWentWrong".tr());
      }
