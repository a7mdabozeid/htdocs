...
define('FILENAME', 'Loungewear July 28th .json');
...
foreach($json as $j_k => $pr_json ) {
    if(empty($pr_json['Parent SKU']) ) {
        ...
        if ( !empty($variant_size) ) {
            $products_to_insert[$prev_sku]['attributes']['pa_size']['options'] = $variant_size;            
        }
        $variant_size = array();
        ...
        $tax_work = 'pa_style-code';
        $tax_value = $pr_json['style code'];
        if( empty($tax_a = get_term_by('name', $tax_value , $tax_work)) || $tax_a === false ) {
            wp_insert_term(
                $tax_value,   // the term 
                $tax_work
        ...
        $variation_data =  array(
            ...
            'stock_qty'     => empty($pr_json['Stock']) ? 5 : $pr_json['Stock'],
