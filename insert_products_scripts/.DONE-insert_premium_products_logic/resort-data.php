...
define('FILENAME', 'CopyOfPremium.json');
...
foreach($json as $j_k => $pr_json ) {
    if(empty($pr_json['Parent SKU']) ) {
        ...
        $tax_work = 'pa_style-no';
        $tax_value = $pr_json['Style No.'];
        if( empty($tax_a = get_term_by('name', $tax_value , $tax_work)) || $tax_a === false ) {
            wp_insert_term(
                $tax_value,   // the term 
                $tax_work
        ...
        $variation_data =  array(
            ...
            'stock_qty'     => empty($pr_json['QTY']) ? 5 : $pr_json['QTY'],
