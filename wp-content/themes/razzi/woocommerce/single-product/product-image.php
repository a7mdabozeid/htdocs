...
$show_video = Razzi\Helper::get_option('product_play_video') == 'load' && is_singular( 'product' ) && $video_position == '1';
?>
<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?>" data-video="<?php echo esc_attr( $video_position ); ?>" data-columns="<?php echo esc_attr( $columns ); ?>" style="opacity: 0; transition: opacity .25s ease-in-out;">
	<?php do_action( 'razzi_before_product_gallery' ); ?>
	...
	<figure class="woocommerce-product-gallery__wrapper">
		...
		if ( $product->get_image_id() ) {
			...
			$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src( 'woocommerce_single' ) ), esc_html__( 'Awaiting product image', 'razzi' ) );
			$html .= '</div>';
		}
