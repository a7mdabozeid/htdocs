...
class License implements Model_Interface , Initiable_Interface {
    ...
    */
    ...
    public function activate_license_notice() {
        ...
        if ( $license_activated !== 'yes' && $notice_dismissed !== 'yes' ) {
            ...
            if ( is_multisite() ) {
                ...
                $agcfw_slmw_settings_url = $current_url . "wp-admin/network/admin.php?page=agcfw-ms-license-settings";

            } else
                $agcfw_slmw_settings_url = admin_url( 'admin.php?page=acfw-license' );
            
            ?>

            <div class="notice notice-error is-dismissible agcfw-activate-license-notice">
                <h4 class="agcfw-activate-license-notice">
                    <?php echo sprintf( __( 'Please <a href="%1$s">activate</a> your copy of Advanced Gift Cards For WooCommerce to get the latest updates and have access to support.' , 'advanced-gift-cards-for-woocommerce' ) , $agcfw_slmw_settings_url ); ?>
                </h4>
            </div>
    ...
    public function ajax_activate_license() {
        ...
        if ( !defined( "DOING_AJAX" ) || !DOING_AJAX )
            $response = array( 'status' => 'fail' , 'error_msg' => __( 'Invalid AJAX Operation' , 'advanced-gift-cards-for-woocommerce' ) );
        elseif ( !isset( $_POST[ 'activation-email' ] ) || !isset( $_POST[ 'license-key' ] ) || !isset( $_POST[ 'ajax-nonce' ] ) )
            $response = array( 'status' => 'fail' , 'error_msg' => __( 'Required parameters not supplied' , 'advanced-gift-cards-for-woocommerce' ) );
        elseif ( !check_ajax_referer( 'agcfw_activate_license' , 'ajax-nonce' , false ) )
            $response = array( 'status' => 'fail' , 'error_msg' => __( 'Security check failed' , 'advanced-gift-cards-for-woocommerce' ) );
        else {
            ...
            if ( empty( $result ) )
                $response = array( 'status' => 'fail' , 'error_msg' => __( 'Failed to activate license. Failed to connect to activation access point. Please contact plugin support.' , 'advanced-gift-cards-for-woocommerce' ) );
            else {

                $result = json_decode( $result );

                if ( empty( $result ) || !property_exists( $result , 'status' ) )
                    $response = array( 'status' => 'fail' , 'error_msg' => __( 'Failed to activate license. Activation access point return invalid response. Please contact plugin support.' , 'advanced-gift-cards-for-woocommerce' ) );
                else {
