...
class Bootstrap implements Model_Interface
{
    ...
    public function plugin_settings_action_link($links)
    {
        if ('yes' !== get_site_option($this->_constants->OPTION_LICENSE_ACTIVATED)) {
            ...
            if (is_multisite()) {
                $license_form_link = current_user_can('manage_network_plugins') ? network_admin_url('admin.php?page=agcfw-ms-license-settings') : '';
            } else {
                $license_form_link = admin_url('admin.php?page=acfw-license#agcfw_admin_app');
            }
