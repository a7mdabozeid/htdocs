...
function action_in_admin_footer() { 
    ...
    if( $pagenow == 'post.php' && !empty( $_GET['action'] ) && $_GET['action'] == 'edit' && $post->post_type == 'shop_order' ) {
        ...
        if ( !in_array( 'administrator', (array) $user->roles ) && !in_array( 'superadmin', (array) $user->roles ) && !in_array( 'shop_manager', (array) $user->roles ) ) {
            echo '
                <script>
                   setInterval(function () {
                    ...
                    if(jQuery("#shipment_infromation input[name=\"order_weight\"]").length > 0 && jQuery("#shipment_infromation input[name=\"order_weight\"]").val() == "0") {
                        jQuery("#shipment_infromation input[name=\"order_weight\"]").val(0.5);
                    }
