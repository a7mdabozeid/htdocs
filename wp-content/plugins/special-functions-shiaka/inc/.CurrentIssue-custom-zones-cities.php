...
function customize_extra_address_fields( $address_fields ) {
    ...
    $address_fields['address_1']['label'] = __('Region', 'shiaka');
    $address_fields['address_1']['options'] = getCustomAramesZones('en');
    ...
    $cities_file = file_get_contents(plugin_dir_url(__FILE__).'collection-cities-'.'en'.'.json');
    ...
    $address_fields['city']['options'] = $new_cities_options;
    ...
    if(!isset($_REQUEST['wc-ajax'])) { 
        ...
        echo '<script>
            setTimeout(function () {
                ...
                jQuery("#billing_country_field").before(jQuery("#billing_email_field"));
                jQuery("#billing_email_field").before(jQuery("#billing_phone_field"));
            
                if(jQuery(".woocommerce-MyAccount-content").length > 0) {
                    jQuery("#billing_company_field").before(jQuery("#billing_address_2_field"));
                }
        ...
        $scripts .= '
            ...
            jQuery(function () {
                ...
                setTimeout(function () {
                    ...
                    // jQuery(".autocomplete-selector-class input").remove();
                    // jQuery(".autocomplete-selector-class .woocommerce-input-wrapper").append(jQuery(_inHTML));
                    function initialize(){
...
jQuery("input[name=\"payment_method\"]").each(function () {              jQuery(document).on("change", "#"+jQuery(this).attr("id") , function () {  jQuery("#ship-to-different-address-checkbox").change();  }); });
    ...
    wp_enqueue_script('<script>'.$scripts.'</script>', '', array('jquery'));
...
add_filter( 'woocommerce_shipping_fields', 'jeroen_sormani_change_city_to_dropdown' );
