...
function add_filter_by_payment_method_orders() {
    ...
    if ( 'shop_order' === $typenow ) {
        ...
        <select name="_shop_order_payment_method" id="dropdown_shop_order_payment_method">
            ...
            <?php foreach ( $gateways as $id => $gateway ) :  if(true):  ?>
...
add_filter( 'woocommerce_get_breadcrumb', function($crumbs, $breadcrumb){
	...
	if($shop_page_id > 0 && is_product() ) { // We check the correctness of the received ID and that the current page is not a store page and that the page is a product page or a product category
		$new_breadcrumb = [
		...
		$new_breadcrumb_home = [
			//_x( 'Shop', 'breadcrumb', 'woocommerce' ), // A string for adding your own shop name. To use uncomment it but comment out the following
			__('Home', 'woocommerce'), // Requesting the name of the shop page
			get_home_url() // Requesting the URL of the shop page
		];
		$new_breadcrumb_pr = [
		    get_the_title(), // Requesting the name of the shop page
			'' // Requesting the URL of the shop page
		];
// 		array_splice($crumbs, 0, count($crumbs), [$new_breadcrumb_pr]); // Add a new item to the breadcrumbs after the "Home" item
    	$crumbs = array(
            $new_breadcrumb_home, 
            $new_breadcrumb_pr
        );
...
function display_custom_product_attributes_on_loop() {
    global $product;
    $generation = $product->get_attribute('Generation');
    // 1959
    $qq_object = get_queried_object();
    if( !isset($_GET['wc-ajax']) ):
        ...
        if (
            ( 
                is_product_category('ثوب-كلاسيك')
                || 
                is_product_category('classic-thob')
                ||
                is_product_category('classic')
                || ( !empty($qq_object) && is_object($qq_object) && isset($qq_object->term_id) && $qq_object->term_id == 1959) 
            )
             
             && ! empty($generation) 
        ) {
            global $wp_query;
           echo '<div class="product-loop-item-categoty-generation-custom-label">
...
function action_in_admin_footer_print_pdf_custom_file_load() { 
    ...
    if( $pagenow == 'edit.php' && $post->post_type == 'shop_order' ) {
...
function script_implement() {
        ...
        if( isset($_GET['testenspacesproductsali']) ) {
            echo '
            <script>
              jQuery("body").addClass("ali");
            </script>
            ';
        }
        //      body {
...
//         }
 ...
 // Cancel form 
    ...
    $script .= '
        <script>
            if( window.location.href.search("cancel_order") > 0  ) {
                window.location.replace("'.home_url().'/order-cancel-form");
            }
            jQuery(".woocommerce-button.cancel").each(function () {
                var base = "'.home_url().'/order-cancel-form";
                var _cancelBtn = jQuery(this);
                var orderNum = jQuery(".woocommerce-orders-table__cell-order-number > a", _cancelBtn.closest(".order-item")).text().trim();
                _cancelBtn.attr("href", base + "?orderidtocancel="+orderNum.substr(1));
                _cancelBtn.attr("style", "display:  inline-block !important");
            });
            
            
            if( jQuery("[name=\"firstname-order-cancel\"]").length > 0 ) {
                    jQuery("[name=\"firstname-order-cancel\"]").closest("form")
                    .append(jQuery(`<input type="hidden" id="your-name" name="your-name" />`))
                    .append(jQuery(`<input type="hidden" id="your-email" name="your-email" />`))
                    .append(jQuery(`<input type="hidden" value="Order Cancel" id="your-subject" name="your-subject" />`));
                    
                    jQuery("input[name=\"orderid-order-cancel\"]").on("keyup", function () {
                        jQuery("#your-subject").val("Order #"+jQuery("[name=\"orderid-order-cancel\"]").val()+" Requested to be Canceled");
                    });
                    
                    jQuery(`[name="firstname-order-cancel"]`).on("keyup", function () {
                        jQuery("#your-name").val(jQuery(this).val() + " " + jQuery(`[name="lastname-order-cancel"]`).val());
                        jQuery("#your-subject").val("Order #"+jQuery("[name=\"orderid-order-cancel\"]").val()+" Requested to be Canceled");
                    });
                    jQuery(`[name="lastname-order-cancel"]`).on("keyup", function () {
                        jQuery("#your-name").val(jQuery(`[name="firstname-order-cancel"]`).val() + " " + jQuery(this).val() );
                        jQuery("#your-subject").val("Order #"+jQuery("[name=\"orderid-order-cancel\"]").val()+" Requested to be Canceled");
                    });
                    jQuery(`[name="email-order-cancel"]`).on("keyup", function () {
    ...
    // 
    if( isset($_GET['orderidtocancel']) ):
    $script .= '
        <script>
            if( jQuery("[name=\"orderid-order-cancel\"]").length > 0 ) {
                jQuery("[name=\"orderid-order-cancel\"]").val("'.$_GET['orderidtocancel'].'");
                
                var anotherLang = jQuery(".razzi-language .language-dropdown ul li:last-of-type > a");
                
                anotherLang.attr("href", anotherLang.attr("href") + "?orderidtocancel=" + "'.$_GET['orderidtocancel'].'");
    ...
    ';
        ...
        $script .= '
            ...
            function changeCitiesWithKeys(i) {
                if( jQuery("select#billing_address_1").val().trim().length <= 0 ) {
                    confirm("'.__('Please choose a valid region and try again', 'shiaka').'");
                }
                var formData = {
...
jQuery("input[name=\"payment_method\"]").each(function () {              jQuery(document).on("change", "#"+jQuery(this).attr("id") , function () {  jQuery("#ship-to-different-address-checkbox").change();  }); });
    ...
    if( jQuery(`[name="name-928"]`).length > 0 ) {
        jQuery(`[name="name-928"]`).closest("form")
        .append(jQuery(`<input type="hidden" id="your-name" name="your-name" />`))
        .append(jQuery(`<input type="hidden" id="your-email" name="your-email" />`))
    ...
    if( jQuery("input[name=\"firstname-refund-guest\"]").length > 0 ) {
        ...
        jQuery("input[name=\"email-refund-guest\"]", _RefundGuestform).on("keyup", function () {
            jQuery("#your-email", _RefundGuestform).val(jQuery(this).val());
        });
