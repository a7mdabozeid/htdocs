...
{
    private $wsdl_link = "https://sms.montymobile.com/API";
    ...
    public function SendSMS()
    {
        ...
        try {
            ...
            $response = wp_remote_post(sprintf('%s/SendBulkSMS', $this->wsdl_link), [
                'headers' => [
            ...
            if (!empty($result) && isset($result['ErrorCode'])) {
                if ($result['ErrorCode'] == '0') {
                    ...
                    // Log the result
                    $this->log($this->from, $this->msg, $this->to, $result['Description'], 'error');
                    // Return error
                    return new \WP_Error('send-sms', $result['Description']);
                }
            }
        } catch (\Exception $e) {
            // Log the result
    ...
    {
        $response = wp_remote_get(sprintf('%s/AccountBalance', $this->wsdl_link), [
            'body' => [
                'username' => $this->username,
                'password' => $this->password,
            ]
