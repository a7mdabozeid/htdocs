 ...
 * @package Razzi
 */
class Socials_Walker extends \Walker_Nav_Menu {
	/**
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$url_name = preg_replace('^(ht|f)tp(s?)\:\/\/[0-9a-zA-Z]([-.\w]*[0-9a-zA-Z])*(:(0-9)*)*(\/?)([a-zA-Z0-9\-‌​\.\?\,\'\/\\\+&amp;%\$#_]*)?$^',' ', $host_url);
		$url_name = preg_replace('/^www\./i', '', $url_name);
		$url_name = preg_replace('/\.com/', '', $url_name);

		if ( $url_name == 'plus.google' ) {
			$url_name = 'google';
		} elseif ( $url_name == 't.me' ) {
			$url_name = 'telegram';
		} elseif ( $url_name == 'web.whatsapp' ) {
			$url_name = 'whatsapp';
		} elseif ( $url_name == 'api.whatsapp' ) {
			$url_name = 'whatsapp';
		}
		...
		if ( $item_type_icon === 'svg' ) {
			...
			$item_svg_html  = ! empty( $item_icon_svg ) ? '<span ' . $item_icon_color . ' class="razzi-svg-icon">' . \Razzi\Icon::sanitize_svg( $item_icon_svg ) . '</span> ' : '';
		} elseif ( ! empty( $item_icon_image ) ) {
			$item_svg_html = '<img src="' . esc_attr( $item_icon_image ) . '">';
		}

		// Svg
		if( empty($item_svg_html) ) {
			$item_svg_html = \Razzi\Icon::get_svg( $url_name, '', 'social' );
		}
