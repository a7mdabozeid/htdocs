...
namespace FRFreeVendor\WPDesk\Dashboard;

final class DashboardWidget
{
    const ID = 'flexible-refund-and-return-order-for-woocommerce';
    ...
    {
        \wp_add_dashboard_widget(self::ID, \__('Grow your business with WP Desk', 'flexible-refund-and-return-order-for-woocommerce'), [$this, 'widget_output'], null, null, 'normal', 'high');
    }
    ...
    public function widget_output()
    {
        ...
        if (!empty($widget_data)) {
            ...
            foreach ($widget_data['plugins'] as $plugin) {
                ...
                echo '<a class="button button-primary button-large" href="' . \esc_url($plugin_url) . '" target="_blank">' . \esc_html__('More info', 'flexible-refund-and-return-order-for-woocommerce') . '</a>';
                echo '&nbsp;';
                echo '<a class="button button-large" href="' . \esc_url($add_to_cart_url) . '" target="_blank">' . \esc_html__('Buy now', 'flexible-refund-and-return-order-for-woocommerce') . '</a>';
                echo '</div>';
                echo '</li>';
            }
            echo '</ul>';
