...
use FRFreeVendor\Monolog\Handler\AbstractHandler;
/**
...
class ErrorHandler
{
    ...
    {
        $this->logger->log($this->uncaughtExceptionLevel === null ? \Psr\Log\LogLevel::ERROR : $this->uncaughtExceptionLevel, \sprintf('Uncaught Exception %s: "%s" at %s line %s', \FRFreeVendor\Monolog\Utils::getClass($e), $e->getMessage(), $e->getFile(), $e->getLine()), array('exception' => $e));
    ...
    public function handleFatalError()
    {
        ...
        if ($lastError && \in_array($lastError['type'], self::$fatalErrors, \true)) {
            ...
            if ($this->logger instanceof \FRFreeVendor\Monolog\Logger) {
                foreach ($this->logger->getHandlers() as $handler) {
                    if ($handler instanceof \FRFreeVendor\Monolog\Handler\AbstractHandler) {
                        $handler->close();
                    }
                }
            }
