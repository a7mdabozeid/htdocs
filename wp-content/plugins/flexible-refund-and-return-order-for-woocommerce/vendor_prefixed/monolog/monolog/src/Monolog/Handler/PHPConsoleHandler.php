...
use FRFreeVendor\PhpConsole\Connector;
use FRFreeVendor\PhpConsole\Handler;
use FRFreeVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \FRFreeVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \FRFreeVendor\PhpConsole\Connector $connector = null, $level = \FRFreeVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('FRFreeVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\FRFreeVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \FRFreeVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \FRFreeVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\FRFreeVendor\PhpConsole\Helper::isRegistered()) {
            \FRFreeVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \FRFreeVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \FRFreeVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \FRFreeVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \FRFreeVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
