...
class ESTP_MailChimp {
	/**
	 * @var string
	 */
	protected $transient_name = 'estp_mailchimp_lists';
	...
	public function get_lists( $force_refresh = false, $force_fallback = false ) {
		...
		if( ! is_array( $cached_lists ) ) {
            ...
            $estp_object = new ESTP_Class();	
            $api = $estp_object->estp_mc_get_api();
	...
	public function get_subscriber_count( $list_ids ) {
		...
		$list_counts = get_transient( 'estp_list_counts' );
		...
		if( false === $list_counts ) {
			...
			if ( is_array( $lists ) ) {
				...
				$transient_lifetime = apply_filters( 'estp_lists_count_cache_time', 1200 ); // 20 mins by default

				set_transient( 'estp_list_counts', $list_counts, $transient_lifetime );
				set_transient( 'estp_list_counts_fallback', $list_counts, 86400 ); // 1 day
			} else {
				// use fallback transient
				$list_counts = get_transient( 'estp_list_counts_fallback' );
				if ( false === $list_counts ) {
					return 0;
				}
			}
		...
		return apply_filters( 'estp_subscriber_count', $count );
	}
	...
	public function get_list_field_name_by_tag( $list_id, $tag ) {
		...
		switch( $tag ) {
			case 'EMAIL':
				return __( 'Email address', APEXNB_TD );
				break;
			case 'OPTIN_IP':
				return __( 'IP Address', APEXNB_TD );
				break;
		}
